...
# ========= PARTIE CONSEILLER (reçoit les mêmes valeurs) =========
else:
    st.title("🛡️ Espace Conseiller")
    seuil_conseiller = st.slider("Seuil de décision (interne conseiller)", 0.05, 0.95, 0.50, 0.01)

    tab1, tab2, tab3 = st.tabs(["📥 Dernière saisie utilisateur", "📊 Analyses", "🧾 Historique & Validation"])

    # --- Tab 1 : Dernière saisie utilisateur (pas de formulaire ici) ---
    with tab1:
        histo = charger_historique()
        en_attente = histo[histo["Statut"] == "En attente"].copy()
        if len(en_attente) == 0:
            st.info("Aucune transaction en attente. Demandez à l'utilisateur de soumettre une transaction.")
        else:
            # on prend la plus récente
            last = en_attente.sort_values("Date").iloc[-1]
            st.markdown("#### Détails de la transaction (saisie utilisateur)")
            details = last[[
                "ID","Date","ClientID","Montant","Heure","HeurePreferee","DeltaHeure","NbTransactions24h",
                "Pays","PaysResidence","Carte","DeviceType","EnLigne","Probabilité","ActionRecommandée"
            ]].to_frame().T
            st.dataframe(details, use_container_width=True, hide_index=True)

            proba = float(last["Probabilité"])
            c1, c2 = st.columns([1,1])
            with c2:
                # Jauge + histogramme = visibles ici
            ...
            with c1:
                ...
                st.write(f"Action recommandée: **{last['ActionRecommandée']}**")
